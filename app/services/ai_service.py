...
def fulfillment_queue(
    ...
    wh = ["1=1"]
    if category:
        wh.append(f'''b.{qcol("sales_order_category")} = '{category.replace(chr(39), chr(39)+chr(39))}' ''')
    if country:
        wh.append(f'''exists (
            select 1 from {qview("orders_header_summary")} h
            where h.{qcol("sales_order_id")} = b.{qcol("sales_order_id")}
              and h.{qcol("customer_country")} = '{country.replace(chr(39), chr(39)+chr(39))}'
        )''')
    where = " and ".join(wh)
    rows = run_query(f'''
        select
            b.{qcol("sales_order_id")} as sales_order_id,
            b.{qcol("customer_name")} as customer_name,
            b.{qcol("sales_order_category")} as sales_order_category,
            b.{qcol("order_age_days")} as order_age_days,
            b.{qcol("days_to_requested_delivery")} as days_to_requested_delivery,
            b.{qcol("open_value")} as open_value,
            b.{qcol("total_open_quantity")} as total_open_quantity,
            b.{qcol("backorder_reasons")} as backorder_reasons,
            b.{qcol("plant_name")} as plant_name,
            b.{qcol("material_key")} as material_key
        from {qview("orders_backlog_analysis")} b
        where {where}
        order by b.{qcol("open_value")} desc
...
def inventory_queue(limit: int = 50, country: str | None = None) -> list[dict]:
    plant_cf = (
        f''' and {qcol("country_code")} = '{country.replace(chr(39), chr(39)+chr(39))}' '''
        if country else ""
    )
    rows = run_query(f'''
        select
...
def forecast_queue(limit: int = 50, country: str | None = None) -> list[dict]:
    plant_cf = (
        f''' and {qcol("country_code")} = '{country.replace(chr(39), chr(39)+chr(39))}' '''
        if country else ""
    )
