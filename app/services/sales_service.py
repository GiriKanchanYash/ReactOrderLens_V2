...
def get_filters() -> dict:
    ...
    statuses = run_query(f'''
        ...
        order by 1
    ''')
    customer_types = run_query(f'''
        select distinct {qcol("customer_type")} as customer_type
        from {qview("customer_profile")}
        where {qcol("customer_type")} is not null
        order by 1
    ''')
    dealers = run_query(f'''
    ...
    ''')
    # Union customer + plant countries for a single Country dropdown
    country_set = []
    seen = set()
    for r in list(countries) + list(plant_countries):
        c = _str(r, "country_code")
    ...
    result = {
        ...
        "customer_types": [_str(r, "customer_type") for r in customer_types],
        "dealers": [
    ...
    try:
        dr = run_query(f'''
            select
                min({qcol("order_date")}) as min_d,
                max({qcol("order_date")}) as max_d
            from {qview("orders_header_summary")}
            where {qcol("order_date")} is not null
        ''')
        if dr:
            result["order_date_min"] = _str(dr[0], "min_d")[:10] or None
            result["order_date_max"] = _str(dr[0], "max_d")[:10] or None
    except Exception:
        pass
...
def get_summary(
    ...
    cache_key = f"summary:{country or 'all'}:{category or 'all'}:{date_from or 'all'}:{date_to or 'all'}"
    cached = _cache_get(cache_key)
    if cached is not None:
        return cached

    cf = f''' and {qcol("customer_country")} = '{_esc(country)}' ''' if country else ""
    catf = f''' and {qcol("sales_order_category")} = '{_esc(category)}' ''' if category else ""
    # Normalize YYYY-MM-DD; ignore invalid
    df = (date_from or "")[:10] if date_from else ""
    dt = (date_to or "")[:10] if date_to else ""
    date_from_f = f''' and {qcol("order_date")} >= '{_esc(df)}' ''' if df else ""
    date_to_f = f''' and {qcol("order_date")} <= '{_esc(dt)}' ''' if dt else ""
    # Backlog has category; country via header join; date on backlog order_date
    backlog_cat = f''' and b.{qcol("sales_order_category")} = '{_esc(category)}' ''' if category else ""
    backlog_country = (
        f''' and exists (
            select 1 from {qview("orders_header_summary")} h
            where h.{qcol("sales_order_id")} = b.{qcol("sales_order_id")}
              and h.{qcol("customer_country")} = '{_esc(country)}'
        ) '''
        if country else ""
    )
    backlog_from = f''' and b.{qcol("order_date")} >= '{_esc(df)}' ''' if df else ""
    backlog_to = f''' and b.{qcol("order_date")} <= '{_esc(dt)}' ''' if dt else ""
    plant_cf = f''' and {qcol("country_code")} = '{_esc(country)}' ''' if country else ""
    dealer_cf = (
        f''' and {qcol("dealer_country", "dealer_order_performance")} = '{_esc(country)}' '''
        if country else ""
    )
    ...
    order_kpis = run_query(f'''
        select
            ...
            min({qcol("order_date")}) as scope_min_date,
            max({qcol("order_date")}) as scope_max_date
        from {qview("orders_header_summary")}
        where 1=1 {cf} {catf} {date_from_f} {date_to_f}
    ''')
    o = order_kpis[0] if order_kpis else {}

    backlog = run_query(f'''
        select
            count(distinct b.{qcol("sales_order_id")}) as backlog_orders,
            coalesce(sum(b.{qcol("open_value")}), 0) as backlog_value,
            coalesce(avg(b.{qcol("order_age_days")}), 0) as avg_age_days
        from {qview("orders_backlog_analysis")} b
        where 1=1 {backlog_cat} {backlog_country} {backlog_from} {backlog_to}
    ''')
    ...
    category_chart = run_query(f'''
        ...
        from {qview("orders_header_summary")}
        where 1=1 {cf} {date_from_f} {date_to_f}
    ...
    trend_chart = run_query(f'''
        ...
        from {qview("orders_header_summary")}
        where {qcol("order_date")} is not null {cf} {catf} {date_from_f} {date_to_f}
    ...
    result = {
        ...
        "date_scope": {
            ...
            "date_to": dt or None,
            "orders_min_date": _str(o, "scope_min_date")[:10] or None,
            "orders_max_date": _str(o, "scope_max_date")[:10] or None,
            "applies_to": ["orders", "backlog", "blocks", "charts"],
            "not_applied_to": ["inventory", "forecast", "dealer_fulfillment"],
            "note": (
                "Date filter uses sales order_date for orders, backlog, blocks, and charts. "
                "Inventory / forecast / dealer KPIs stay current snapshot (country still applies)."
            ),
        },
    }
...
def get_orders(
    ...
    wh = ["1=1"]
    if customer:
        wh.append(f'''{qcol("customer_name")} = '{_esc(customer)}' ''')
    if status:
        wh.append(f'''{qcol("overall_order_status")} = '{_esc(status)}' ''')
    if category:
        wh.append(f'''{qcol("sales_order_category")} = '{_esc(category)}' ''')
    if country:
        wh.append(f'''{qcol("customer_country")} = '{_esc(country)}' ''')
    df = (date_from or "")[:10] if date_from else ""
    dt = (date_to or "")[:10] if date_to else ""
    if df:
        wh.append(f'''{qcol("order_date")} >= '{_esc(df)}' ''')
    if dt:
        wh.append(f'''{qcol("order_date")} <= '{_esc(dt)}' ''')
    where = " and ".join(wh)
    rows = run_query(f'''
        select
...
def get_customer_health(
    ...
    wh = ["1=1"]
    if status:
        wh.append(f'''r.{qcol("customer_status")} = '{_esc(status)}' ''')
    if customer_type:
        wh.append(f'''r.{qcol("customer_type")} = '{_esc(customer_type)}' ''')
    if country:
        wh.append(f'''p.{qcol("country_code")} = '{_esc(country)}' ''')
    where = " and ".join(wh)
    rows = run_query(f'''
        select
            ...
            p.{qcol("country_code")} as country_code
        ...
        left join {qview("customer_profile")} p
          on r.{qcol("customer_key")} = p.{qcol("customer_key")}
        where {where}
    ...
    summary = run_query(f'''
        select
            SUM(CASE WHEN r.{qcol("customer_status")} = 'Active' THEN 1 ELSE 0 END) as active_count,
            SUM(CASE WHEN r.{qcol("customer_status")} = 'At Risk' THEN 1 ELSE 0 END) as at_risk_count,
            SUM(CASE WHEN r.{qcol("customer_status")} = 'Inactive' THEN 1 ELSE 0 END) as inactive_count,
            avg(r.{qcol("days_since_last_order")}) as avg_days_since
        from {qview("customer_recency_frequency")} r
        left join {qview("customer_profile")} p
          on r.{qcol("customer_key")} = p.{qcol("customer_key")}
        where {where}
    ...
    return {
        ...
        "insights": {
            ...
            "recommended_actions": [
                "Call / visit At Risk customers with highest CLV this week (At Risk = 91–365 days since last order).",
                "Use View orders on a customer row to see their order history — not the unfiltered Operations list.",
                "Log outreach so the team knows who was contacted.",
                "Review Inactive accounts (>365 days) for win-back offers or account closure.",
            ],
        },
        "rows": [
            {
...
def get_inventory_overview(country: str | None = None, limit: int = 100) -> dict:
    plant_cf = f''' and {qcol("country_code")} = '{_esc(country)}' ''' if country else ""
    plants = run_query(f'''
        select
    ...
    return {
        ...
        "insights": {
            "headline": (
                f"{len(breaches)} reorder breaches{scope} · shortage qty {total_shortage:,.0f} · "
...
def get_forecast_overview(country: str | None = None, limit: int = 100) -> dict:
